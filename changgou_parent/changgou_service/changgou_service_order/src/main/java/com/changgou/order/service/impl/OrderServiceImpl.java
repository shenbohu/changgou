...
public class OrderServiceImpl implements OrderService {
    ...
    public String add(Order order) {
        ...
        //3:填充订单数据 保存 tb_order
        int count = orderMapper.insertSelective(order);
        //4:填充订单想的数据 保存 tb_order_item
        ...
        skuFeign.decerCount(order.getUsername());
        //添加任务数据
        System.out.println("添加任务数据");
        Task task = new Task();
    ...
    public void autoTack() {
        //读取订单配置信息  根据主键   表中的 时间信息
        OrderConfig orderConfig = orderConfigMapper.selectByPrimaryKey(1);
        //获得时间节点 LocalDate 只会有日期 没有时间
        LocalDate now = LocalDate.now();//当前日期

        //获取过期的时间节点，在这个日期前发货的未收货订单都
        // now.plusDays 当前日期＋多少天
...
//            take(order.getId(),"system" );
    ...
    public void confirmTask(String orderId, String operator) {
        ...
        orderLog.setConsignStatus("2"); //已送达
    ...
    public void batchSend(List<Order> orders) {
        ...
        for (Order order : orders) {
            ...
            if (order.getShippingCode() == null || order.getShippingName() == null) {
                throw new RuntimeException("请输入运单号和物流公司名称");
            }
        }
